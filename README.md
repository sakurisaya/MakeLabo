...
- **SQLAlchemy / PostgreSQL**: データベース操作を安全に行うためORMを利用しています。本番環境のデータベースとしてAmazon RDS（PostgreSQL）を採用し、堅牢なデータ管理を実現しています。
...
- **Amazon RDS**: フルマネージドなデータベース（PostgreSQL）を採用し、将来的なアクセス増加にも柔軟に対応できる構成にしています。
...
### 5. データベースの分離によるデータの永続化と保守性
当初はアプリケーションサーバー内蔵のDBを使用していましたが、デプロイのたびにデータが初期化される課題がありました。これを解消するため、Amazon RDS（PostgreSQL）を導入してDBをサーバーから分離。本番環境にふさわしい「データが消えない、信頼性の高い構成」へと改善しました。
